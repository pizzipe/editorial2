...
export class Sidebar implements AfterViewInit {
  ...
  checkScreenSize() {
    this.isInactive = window.innerWidth <= 1280; // FIXME: media query non costante
  }
  ...
  private updateStickyBehavior() {
    ...
    // Se siamo su mobile/tablet (sidebar in overlay), non serve lo sticky
    if (this.isInactive || window.innerWidth <= 1280) { // FIXME: media query non costante
      return;
    }
